...
import dgl.backend.pytorch as torch_tensor
import numpy as np
import torch
...
def test_take():
    data = np.random.random([3, 4])
    torch_data = torch.Tensor(data)
    np_out = np_tensor.take(data,1,1)
    torch_out = torch_tensor.take(torch_data,torch.tensor(1),1).numpy()
...
def test_nonzero_1d():
    ...
    assert np_out == torch_out, "nonzero_1d fail"
...
def test_sort_1d():
    ...
    torch_indexs = torch_out.indices.numpy()
    assert np.max(np.abs(np_indexs - torch_indexs))<=max_error, "sort_1d fail"
...
def test_rand_shuffle():
    ...
    assert np.max(np.abs(np_out - torch_out))<=max_error, "rand_shuffle fail"
