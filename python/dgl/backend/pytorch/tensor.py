...
class BinaryReduce(th.autograd.Function):
    ...
    def forward(ctx, reducer, binary_op, graph, lhs, rhs, lhs_data, rhs_data, out_data,
        ...
        if reducer == 'mean':
            degs = lhs_data.new_zeros((out_data.shape[0],))
    ...
    def backward(ctx, grad_out):
        ...
        if ctx.needs_input_grad[5]:
            grad_lhs = grad_out.new_zeros((lhs_data_nd.shape[0],) + feat_shape)
        ...
        if ctx.needs_input_grad[6]:
            grad_rhs = grad_out.new_zeros((rhs_data_nd.shape[0],) + feat_shape)
...
def binary_reduce(reducer, binary_op, graph, lhs, rhs, lhs_data, rhs_data,
    ...
    if binary_op == 'dot':
        out_shape = feat_shape[:-1]
    out_data = lhs_data.new_zeros((out_size,) + out_shape)
...
class CopyReduce(th.autograd.Function):
    ...
    def forward(ctx, reducer, graph, target, in_data, out_data, out_size, in_map,
        ...
        if reducer == 'mean':
            ...
            degs = in_data.new_zeros((out_data.shape[0],))
            in_ones_nd = zerocopy_to_dgl_ndarray(in_ones)
            degs_nd = zerocopy_to_dgl_ndarray(degs)
            K.copy_reduce(
    ...
    def backward(ctx, grad_out):
        ...
        if ctx.needs_input_grad[3]:
            grad_in = grad_out.new_zeros(in_data_nd.shape)
...
def copy_reduce(reducer, graph, target, in_data, out_size, in_map=(None, None),
                out_map=(None, None)):
    out_data = in_data.new_zeros((out_size,) + in_data.shape[1:])
