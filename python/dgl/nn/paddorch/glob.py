...
from ...backend import paddorch as F
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mem, lengths_x, lengths_mem):
        ...
        # generate mask
        mask = th.zeros(batch_size, max_len_x, max_len_mem)
...
class InducedSetAttentionBlock(nn.Module):
    ...
    def forward(self, feat, lengths):
        ...
        query = th.repeat(self.inducing_points,batch_size, 1)
        # query =  self.inducing_points.expand(batch_size, 1)
...
class PMALayer(nn.Module):
    ...
    def forward(self, feat, lengths):
        ...
        """
        batch_size = len(lengths)
        query=paddorch.repeat(self.seed_vectors,(batch_size, 1))
