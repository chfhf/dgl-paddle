...
class EdgeSoftmax(th.autograd.Function):
    ...
    def forward(self, g, score, eids):
        ...
        if not score.stop_gradient:
            self.register_hook(score,"score")
        else:
            self.delete_hook("score")
        score.stop_gradient = True ## I dont want to compute score.grad during copy_reduce, binary_reduce
        ...
        # operators.
        score_device=str(score.place)
        score_context = utils.to_dgl_context(score_device)
        ...
        self.backward_cache = n_nodes, n_edges, gidx
        ...
        self.save_for_backward(out,score)
        out.register_hook(self.backward)  # need add this line before the out+th.mean line
        score.stop_gradient = False
        out = out + th.mean(score ) * 0  ##trick to force create a connected graph!
    ...
    def backward(self, grad_out):
        ...
        """
        n_nodes, n_edges, gidx = self.backward_cache
        out,score = self.saved_tensors

        #g.edata['grad_s'] = out * grad_out
        grad_s = out * grad_out
        grad_s.stop_gradient=True
        ...
        self.grad_cache["score"]=grad_score
        del out
        return grad_out
