...
class GMMConv(nn.Module):
    ...
    def reset_parameters(self):
        ...
        init.constant_(self.inv_sigma, 1)
    ...
    def forward(self, graph, feat, pseudo):
        ...
        with graph.local_scope():
            ...
            gaussian = -0.5 * ((pseudo.view(E, 1, self._dim) -  th.reshape(self.mu,(1, self._n_kernels, self._dim))) ** 2)  #       self.mu.view(1, self._n_kernels, self._dim
            # gaussian = gaussian * (self.inv_sigma.view(1, self._n_kernels, self._dim) ** 2)
            gaussian = th.convertTensor(gaussian * (th.reshape(self.inv_sigma,(1, self._n_kernels, self._dim)) ** 2))
